...
- [ ] 真实 VLM 通过 LiteLLM `vision_understanding` 能力调用，输出经过 schema 校验和 taxonomy 归一化；真实 FashionSigLIP embedding 写入 pgvector，失败时保留 Capture 并可重试。
- [ ] 代码按能力纵向组织，并在能力内保持 domain/application/infrastructure/interface 依赖方向；静态检查阻止 provider DTO、FastAPI、SQLAlchemy、Celery 或 LiteLLM 进入领域层。
- [ ] 豆包/方舟密钥仅从服务端环境注入；产品代码不直接引用具体模型调用，前端、fixture、trace 和日志均不含密钥。
...
- [ ] 建立至少 30 条有来源和使用说明的本地 Feed 素材库，覆盖单品、整套、配饰、遮挡、运动、低对比和负例，并划分固定回归子集；需要的预标注由开发阶段人工完成并记录为 `curated_seed`，不消耗模型 API、不冒充 AI 结果。
...
- [ ] 审美重排和解释通过 LiteLLM `reasoning` 能力调用；硬约束、召回、状态迁移和购买清单不委托给模型。
...
- [ ] 每个可见主链状态均有真实移动端操作截图及输入/trace 证据，覆盖交互、processing、success、failure 和 recovery；仅 DOM 断言不能通过验收。
