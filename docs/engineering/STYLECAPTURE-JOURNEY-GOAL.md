...
Task 2–10 必须按 ExecPlan 的 M1–M7 依赖顺序连续完成 branch-local milestone ExecPlans 和 SDD task briefs，不以 M0 招募、付款、成熟分母、生产转化或规模指标作为开发、TestFlight 或 aggregate Goal 完成的硬门，也不在里程碑之间等待人工确认。现有 M0 研究脚本、证据 schema、外部 readiness 和商业指标仅作为未来可选的市场验证与运营参考；缺少招募账号/渠道、法律主体/联系人、商户/退款权限或外部证据库不阻断 App/backend 实现、Apple sandbox、staging、TestFlight 或发布就绪验证。技术、sandbox、staging 与 TestFlight 证据不得伪装成真实生产收入或用户行为证据。除非未来得到明确授权，不得读取、创建、编辑、评论、关闭或以其他方式触碰 GitHub Issues 或 PRs。
...
完成 stop condition：Task 2–10 与 M1–M7 的中国区可发布版本全部交付；¥12 Journey 包的 Day 1 免费、购买、恢复、退款撤销、离线、账户删除、对账和回滚闭环均有新鲜自动化与 Apple sandbox/TestFlight 证据；真实 provider 主链路、Promptfoo/OTel/受控 Langfuse、privacy canary 与删除传播通过；无 unresolved P0/P1；最终 spec、reuse/license、architecture、security/privacy、code quality、conversion UX 六类审查全部 `APPROVE + CLEAR`。PRD 中的转化、复购、VSS、毛利与退款指标保留为上线后的经营观测与优化信号，不是本 aggregate Goal 的完成硬门，也不得导致 Tasks 2–10 停止开发。订阅不是 P0 默认 CTA，年订阅只在未来明确产品决策后展示。
```
...
```text
审计同一 StyleCapture Journey Goal 的当前执行状态。读取 active Goal、docs/exec-plans/0043-stylecapture-journey-commercial-app.md、当前 milestone ExecPlan、当前 SDD task brief、相关 ADR、最近 diff/commit、测试、OpenAPI/generated client、Promptfoo/Langfuse/OTel 证据、iOS 截图/录屏和资源状态。不得读取、创建、编辑、评论、关闭或以其他方式触碰 GitHub Issues 或 PRs，除非未来得到明确授权。判断工作是否仍直接推进 3–7 天付费旅行结果以及 Tasks 2–10 全量交付；检查是否混入单日场合或订阅抢跑，遗漏验收、隐藏 mock/fixed result、测试放宽、手写可生成合同、重复框架/算法、无依据抽象/依赖、provider 直连、敏感 trace/event、跨账户/StoreKit/删除/AI 标识漂移、失败/恢复 UX、成本/毛利，或把 sandbox/staging/TestFlight 技术证据误报为生产行为。M0 招募、付款、成熟分母和经营规模指标不是开发或 Goal 完成硬门，不得据此暂停或 blocked。检查 CPU、memory pressure、swap、thermal、disk、Docker 和重复 watcher；触发护栏时降并发或停止昂贵本地任务。对安全且在当前 SDD task brief/ExecPlan 范围内的 P0/P1 立即修复并运行受影响验证；需要持久跨切面决策时更新 ADR，需要当前结果的发现直接修订 SDD task brief/ExecPlan，不创建逃避验收的 branch-local follow-up。然后继续当前 milestone。仅报告实质修正、精确阻塞或简洁 clean audit；Goal 完成或分支暂停时禁用本 automation。
```
