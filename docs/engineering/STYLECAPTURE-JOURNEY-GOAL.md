...
严格分离技术开发门与市场商业化门：M0 在 7 天内完成招募和统一 ¥12 报价，最终门只在至少 15 名 plan recipients 达到 `trip_end+7d` 后判断，并记录实际 maturity cutoff。招募 20–30 名未来 30 天有真实 3–7 天旅行的 ICP；pain denominator ≥20 且 ≥60% 评分 ≥7/10；全部收到完整计划和唯一 offer 的用户构成 real-paid denominator（≥15），真实可退款付款/订金 rate ≥33% 且 payer ≥5；`trip_end+7d` 已成熟的全部 plan recipients 构成 execution denominator（≥15），≥50% 至少一天采用计划主/备选 Look 或符合原硬约束的可追踪局部替换，未回访按未执行。三项同时通过才可记录 M0 `GO`，并作为生产商业化、付费生产扩量、scale claim、`PIVOT`/`STOP` 结论和 aggregate Goal 完成的市场证据；`real_paid` 不接受意愿、口头或“等价”承诺，研究收款不得变成 iOS App 外链支付。缺少授权法律主体/联系人、账号/渠道、商户/退款权限或受控外部证据库时，只阻断需要这些输入的真实外部 M0 招募、收款、退款和证据留存操作，不阻断本地 iOS/backend 实现、Apple sandbox、staging 或 TestFlight 技术验证。Task 2–9 可按 ExecPlan 的 M1–M7 和 Implementation Plan 的 Task 2–10 依赖顺序创建/执行 branch-local milestone ExecPlans 和 SDD task briefs，持续推进，不在里程碑之间等待人工确认；任何 sandbox/TestFlight/staging 证据只能证明技术可行，绝不能计入 M0、生产商业化、WTP、收入、退款、毛利或规模分母。M0 未通过或真实证据达到失败条件时记录 PIVOT/STOP，不得用增加场景或功能绕过需求证据。除非未来得到明确授权，不得读取、创建、编辑、评论、关闭或以其他方式触碰 GitHub Issues 或 PRs。
