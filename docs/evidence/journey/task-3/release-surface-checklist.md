...
- [x] The application target exact-pins and explicitly links `swift-sharing` `2.9.1`, and `AppFeature` imports `Sharing` because it directly owns `@Shared(.fileStorage)` symbols. This is the narrow XcodeGen/Xcode 26 link correction observed in run `30393350600`; it does not create another state or persistence framework.
