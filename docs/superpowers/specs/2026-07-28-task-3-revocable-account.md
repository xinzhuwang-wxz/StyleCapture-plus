...
- GREEN: a behavior-first generator regression proves the canonical H5 OpenAPI keeps FastAPI 3.1 nullable unions while the deterministic Swift projection removes only exact `T | null` pairs. The iOS generator input now retains optional fields and Authorization headers instead of silently dropping them under Apple Swift OpenAPI Generator `1.13.0`.
- OPEN: compile the projected generated Swift client, complete live AuthenticationServices/Product API/throwing-Keychain composition, run the final iOS simulator/hosted proof, capture visual evidence and pass the final six-review gate.
