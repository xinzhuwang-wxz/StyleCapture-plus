...
| Apple identity token | [PyJWT](https://github.com/jpadilla/pyjwt) `[crypto]` + Apple fixed JWKS/REST | `2.13.0` / `7144e4534c34810f4525dc4578a32addd8212cff`；复用 lock 中 `cryptography 46.0.7`、`httpx 0.28.1` | MIT；cryptography Apache-2.0/BSD-3-Clause，httpx BSD-3-Clause | 直接/薄适配复用 RS256 验签与 ES256 client-secret 签名；固定 issuer/audience/algorithm/JWKS endpoint，不接受 token 控制 key URL；nonce、authorization-code replay、canonical subject binding、session rotation/revocation/deletion 属于应用策略，应用层只依赖 AppleIdentityVerifier，禁止自写或新增第二套 JOSE |
