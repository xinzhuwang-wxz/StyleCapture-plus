...
**Status:** Adapted from the isolated PR #10 experiment into the product as a
lazy-loaded first-level `像素世界` destination. Local verification only; deployment is
intentionally deferred until the user approves the local result.

## Observable outcome

A mobile-first pixel world entered from the product's first-level navigation. It keeps
the four authored animated characters from PR #10 and appends every succeeded wardrobe
`pixel_cover` artifact as a playable Look. A user can change Look, tap to move, walk
the runway, enter a true full-window / best-effort landscape mode, and return to the
same digital wardrobe without losing product state.
...
It does not add a live community backend, real-time presence, or a general game
framework.
...
- **Product integration:** `像素世界` is a first-level tab next to the existing product
  destinations. The module is code-split, so Feed and wardrobe startup do not pay the
  Canvas/world cost. Back returns to the wardrobe.
- **Wardrobe truth:** the original PR #10 characters remain available. Real wardrobe
  Looks are derived only from succeeded `pixel_cover` RenderArtifacts and retain their
  Look ids; the image artifact is reused rather than regenerated in the browser.
- **Motion parity:** authored four-pose Looks use their idle/walk/cheer/wave artwork.
  API-provided one-image Looks use the same simulation state machine and a procedural
  head/torso/lower-body rig, so move/runway/greet actions remain consistent.
...
| Product wardrobe Looks | Existing `Look` + succeeded `RenderArtifact(kind=pixel_cover)` API contracts | Direct reuse | Preserves the real wardrobe as source of truth and avoids a second pixel database | This repository at product base `435bceb`; project-owned |
| Initial-load performance | React `lazy` + `Suspense` | Direct reuse | Keeps the 67 kB world chunk out of the Feed/wardrobe startup bundle | React 18; MIT |

## Acceptance checklist

- [x] First-level `像素世界` navigation opens the integrated experience and returns.
- [x] Original four animated characters remain available.
- [x] Every succeeded wardrobe pixel cover is appended without resetting local/social state.
- [x] API-provided single-image Looks follow the same move/runway action state machine.
- [x] Full-window mode works immediately even when native fullscreen stalls or drops.
...
- [ ] Commit and push the integrated feature branch without deploying it.
...
- [x] 2026-07-26: Adapted PR #10 into first-level product navigation, connected all
  succeeded wardrobe pixel covers, retained the original animated pose pack, and added
  stable return/fullscreen behavior.
- [x] 2026-07-26: Verified the real Feed → wardrobe → pixel world → change Look → move
  → runway → fullscreen/landscape → return journey at 390×844, including screenshots.
- [x] 2026-07-26: Split the world into a 67 kB lazy chunk, reducing the initial JS from
  508.23 kB to 442.71 kB while keeping the feature available on demand.
...
- Native Fullscreen promises can stall or immediately unwind in H5 containers. Product
  immersive layout therefore owns the visible state; native fullscreen and orientation
  lock are non-blocking enhancements only.
...
- 2026-07-26 — Promote the world to a first-level destination after explicit user
  direction, but lazy-load it so the primary Feed experience remains fast.
- 2026-07-26 — Use existing wardrobe `pixel_cover` artifacts as playable Looks; do not
  invent another store, regenerate covers, or duplicate Look identity.
