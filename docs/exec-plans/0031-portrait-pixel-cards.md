...
- Update `photo.pixel_trial` and `look.pixel_cover` prompts and image-generation requests to use a provider-supported 3:4 portrait canvas (`1728x2304`).
...
1. Add explicit portrait-ratio language and `1728x2304` requests to the two person-card generation paths; increment their prompt versions.
