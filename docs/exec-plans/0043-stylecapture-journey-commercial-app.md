...
- [x] Task 3 replacement diagnosis: run `30394425892` at `4c045dd` passed Python/PostgreSQL architecture and behavior, generated API contract, iOS bootstrap, dependency resolution and package graph. It exposed two later gates: a real Feed early-timer stall and Swift 6/Xcode 26 test-source incompatibilities. The Feed timer now re-arms to its exact wall-clock deadline with a RED/GREEN regression and `247/247` H5 tests passing under one-worker resource limits; the three affected iOS test files preserve their assertions while evaluating actor values outside XCTest autoclosures. A new frozen hosted candidate remains required.
...
- Hosted run `30394425892` showed that replacing a fixed test sleep with an awaited query did not address the underlying Feed race: a platform timer may wake just before its wall-clock deadline, the domain correctly refuses to settle, and unchanged React effect dependencies then leave the session collecting forever. The production effect must re-arm only for the remaining interval; do not inflate the 700ms product policy or weaken the test timeout.
- The same run showed that Swift syntax parsing cannot certify XCTest actor/autoclosure compatibility. Async actor values must be awaited into named locals before `XCTAssert*`; hosted Xcode compilation remains authoritative for these test modules.
