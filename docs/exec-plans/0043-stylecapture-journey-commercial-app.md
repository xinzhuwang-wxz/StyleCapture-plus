...
- 2026-07-29: Treat Sign in with Apple revocation and account deletion as one server-owned lifecycle. Retain the minimum Apple provider grant behind encrypted server storage, call Apple's token revocation endpoint during account deletion before local session-family revocation, and represent `accepted`/`frozen` deletion as processing. Only verified cross-system erasure may be described as completed deletion.
