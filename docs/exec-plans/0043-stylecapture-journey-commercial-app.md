...
- [ ] Hosted macOS Task 2 verification: the first branch-dispatched run resolved every exact SwiftPM pin and reached the simulator step; GitHub's `macos-26` image exposes iPhone 17/17 Pro/17e/Air devices rather than iPhone 16, so CI now targets the available iPhone 17 with the latest installed runtime. Task 2 remains incomplete until that run compiles and passes XCTest/XCUITest.
- [ ] Hosted Xcode trust gate: the second branch-dispatched run selected iPhone 17 and entered build preparation, then Xcode rejected the exact-pinned Apple OpenAPI build plugin because non-interactive CI had not granted plugin trust. CI now passes `-skipPackagePluginValidation` and `-skipMacroValidation` only after exact Package.resolved seeding and post-build byte verification; this removes the interactive prompt without permitting floating packages.
