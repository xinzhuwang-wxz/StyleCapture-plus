...
export type CommunityAvatarSource = {
  /** Stable wardrobe Look identity; omitted for one-off local uploads. */
  lookId?: string;
  assetUrl: string;
  label: string;
  kind:
...
function myLook(source: CommunityAvatarSource, index = 0): PartyLook {
  ...
  return {
    ...
    description:
      source.description ??
      "把你的像素搭配带到主题舞台，让这套 Look 成为今晚的主角。",
    outfitFormula: [source.label || `我的像素 Look ${index + 1}`, "主题舞台", "分享卡"]
  };
}

function normalizeSources(
  sources?: CommunityAvatarSource | readonly CommunityAvatarSource[]
): readonly CommunityAvatarSource[] {
  if (!sources) return [];
  return Array.isArray(sources)
    ? sources
    : [sources as CommunityAvatarSource];
}

function wardrobeLooks(
  sources?: CommunityAvatarSource | readonly CommunityAvatarSource[]
): PartyLook[] {
  return normalizeSources(sources).map((source, index) => myLook(source, index));
}

export function createCommunityScene(
  avatarSources?: CommunityAvatarSource | readonly CommunityAvatarSource[]
): CommunityScene {
  const catalogue = [...animatedLooks, ...curatedLooks];
  const ownedLooks = wardrobeLooks(avatarSources);
  const looks = [...catalogue, ...ownedLooks];
  ...
  const worn = ownedLooks[0]?.id ?? curatedLooks[0].id;
  return {
    theme: {
      title: "花房夜宴",
...
/**
 * Refreshes public wardrobe pixel covers without resetting the room's social
 * state. Curated residents always remain available, and a local upload is kept
 * until the user replaces it explicitly.
 */
export function syncCommunityWardrobeLooks(
  scene: CommunityScene,
  sources: readonly CommunityAvatarSource[]
): CommunityScene {
  const catalogue = [...animatedLooks, ...curatedLooks];
  ...
  return {
    ...scene,
    looks,
    wornLookId: ids.has(scene.wornLookId) ? scene.wornLookId : fallbackId,
    selectedLookId: ids.has(scene.selectedLookId)
      ? scene.selectedLookId
      : fallbackId
  };
}
...
export function replaceMyLook(
  ...
  const replacement = myLook({ ...source, lookId: undefined });
