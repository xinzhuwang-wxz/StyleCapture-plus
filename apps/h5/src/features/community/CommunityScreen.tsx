...
type CommunityScreenProps = {
  ...
  avatarSources?: readonly CommunityAvatarSource[];
  onExit?: () => void;
...
export function CommunityScreen({
  ...
  const initialSources = avatarSources ?? (avatarSource ? [avatarSource] : []);
  const [scene, setScene] = useState<CommunityScene>(() =>
    createCommunityScene(initialSources)
  );
  ...
  const wardrobeSourceSignature = (avatarSources ?? [])
    .map((source) => `${source.lookId ?? ""}:${source.assetUrl}`)
    .join("|");

  useEffect(() => {
    if (!avatarSources) return;
    setScene((current) =>
      syncCommunityWardrobeLooks(current, avatarSources)
    );
    // The signature deliberately avoids resetting the world when the parent
    // re-renders with equivalent API data.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [wardrobeSourceSignature]);
  ...
  /**
   * Full-screen the world. The layout state is the product contract because
   * the browser Fullscreen API is unavailable or short-lived in many H5
   * containers. Native fullscreen and landscape are best-effort bonuses and
   * never drive the React state back to false.
   */
