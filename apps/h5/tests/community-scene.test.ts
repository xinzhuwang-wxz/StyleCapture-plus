...
describe("theme party scene", () => {
  ...
  it("keeps original animated characters and adds every wardrobe pixel cover", () => {
    const scene = createCommunityScene([
      {
        lookId: "look-feed",
        assetUrl: "/v1/looks/look-feed/renders/pixel/image",
        label: "Feed 穿搭 1",
        kind: "public-render-artifact"
      },
      {
        lookId: "look-owned",
        assetUrl: "/v1/looks/look-owned/renders/pixel/image",
        label: "我的穿搭 2",
        kind: "public-render-artifact"
      }
    ]);
    ...
    expect(scene.looks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "wardrobe-look-feed",
          title: "Feed 穿搭 1",
          sourceLabel: "我的公开像素 Look"
        }),
        expect.objectContaining({
          id: "wardrobe-look-owned",
          title: "我的穿搭 2",
          sourceLabel: "我的公开像素 Look"
        })
      ])
    );
    expect(scene.wornLookId).toBe("wardrobe-look-feed");
  });

  it("refreshes wardrobe covers without discarding local uploads or social state", () => {
    const uploaded = replaceMyLook(
      createCommunityScene(),
      {
        assetUrl: "blob:local",
        label: "local.png",
    ...
    const base = reactToSelectedLook(
      selectPartyLook(uploaded, "curated-sweet"),
      "layering"
    );
    const synced = syncCommunityWardrobeLooks(base, [
      {
        lookId: "look-new",
        assetUrl: "/v1/looks/look-new/renders/pixel/image",
        label: "我的穿搭 1",
        kind: "public-render-artifact"
      }
    ]);

    expect(lookById(synced, MY_LOOK_ID)?.assetUrl).toBe("blob:local");
    expect(lookById(synced, "wardrobe-look-new")?.assetUrl).toContain("look-new");
    expect(synced.selectedReaction).toBe("layering");
  });
