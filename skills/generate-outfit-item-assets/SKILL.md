...
- 两条路径都必须输出精确 3:4、1728×2304 和 `#FFFFFF` 四角。AI 路径的提示词必须要求平坦数字纯白背景，禁止灰白、米白、纸张纹理、污点、斑块、渐变、暗角及接触阴影；生成后先从商品轮廓构造保护蒙版，再只把蒙版外确定性清成 `#FFFFFF`。禁止在商品蒙版内按“接近白色”全局漂白，避免白衣、银饰和浅色鞋被背景清理误删。
...
- 只用文字提示词约束像素风格，不向图像模型附加风格参考图。上传图或 Item 图只用于保留目标单品的真实颜色、版型和结构。
- 模型只生成纯净浅底上的像素单品主体，不生成卡片圆环、阴影、边框或装饰。后端从蜜桃、丁香紫、晴空蓝、薄荷绿、奶油黄和莓果粉中稳定选择柔和色板，合成无模糊的低对比中心色块、阶梯双线四角与固定但非对称的装饰布局；每张卡只保留 4–7 个小装饰，默认 6 个，边框节点不计入数量。
- 后端在 256×256 逻辑像素网格完成卡面合成，再用最近邻放大到 1024×1024；不得使用高斯光晕或全图重着色制造灰黑环。
