...
function assertCollage(render) {
  if (!render || typeof render !== "object" || typeof render.id !== "string") {
    throw new Error("Product API returned an invalid render artifact");
  }
  if (render.kind !== "collage") {
    throw new Error(`Product API returned ${render.kind || "an unknown kind"}, expected collage`);
  }
  return render;
}

async function renderLookCollage(lookId, options = {}) {
  ...
  if (!response.ok) await productApiError(response, "collage creation failed");
  const render = assertCollage(await response.json());
  if (!options.wait) return render;
  while (!new Set(["succeeded", "failed", "degraded"]).has(render.status)) {
    await new Promise((resolve) => setTimeout(resolve, 500));
    const latest = await fetch(`${baseUrl}/v1/render-artifacts/${encodeURIComponent(render.id)}`, {
      headers: { cookie },
      signal,
    });
    if (!latest.ok) await productApiError(latest, "collage status lookup failed");
    Object.assign(render, assertCollage(await latest.json()));
  }
  return render;
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!key.startsWith("--") || !value || value.startsWith("--")) {
      throw new Error("Usage: node scripts/render-look-collage.js --look-id <UUID> [--wait] [--api-base-url URL] [--timeout-ms 90000]");
    }
...
async function main(argv) {
  ...
  const output = await renderLookCollage(args["look-id"], {
...
module.exports = { normalizeLookId, renderLookCollage };
